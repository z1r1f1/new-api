...
- Trigger: changes to channel creation, admin channel routes, credential import
  parsing, or the default frontend create-channel drawer's file-import path.
...
- Frontend verification: `cd web/default && bun run typecheck`; run lint when
  the local ESLint dependency graph can execute.
...
- Trigger: changes to the in-app messaging page's emoji reactions, chat message
  API responses, chat WebSocket event payloads, or chat-message persistence.
- Reactions are mutable user metadata, not message-body content. Do not encode
  reaction state into `ChatMessage.Body`.

#### 2. Signatures

- DB model: `model.ChatMessageReaction` stored in `chat_message_reactions`.
- Unique key: `(conversation_id, message_id, user_id, emoji)`.
- Service entrypoint:
  `(*chat.Service).ToggleMessageReaction(ctx, currentUserID, conversationID, messageID, emoji) (*MessageResponse, bool, error)`.
- HTTP endpoint:
  `POST /api/chat/conversations/:id/messages/:message_id/reactions`.
- Request body:
  `{"emoji":"👍"}`.
- Message response field:
  `reactions: [{ emoji: string, count: number, reacted_by_me: boolean }]`.
- WebSocket event type: `message.reaction.updated`.

#### 3. Contracts

- A chat member toggles one emoji reaction for one non-revoked message. Calling
  the endpoint once adds the reaction; calling it again with the same emoji
  removes that user's reaction.
- `MessageResponse.reactions` is always present. Use an empty array when a
  message has no reactions so frontend code does not need null checks.
- `reacted_by_me` is scoped to the requesting user for HTTP list/detail/mutation
  responses.
- Broadcast `message.reaction.updated` events must include aggregate counts and
  operation metadata (`user_id`, `reaction_emoji`, `reaction_active`). Do not
  broadcast another user's `reacted_by_me=true` view to every subscriber.
- The frontend must merge reaction WebSocket events by preserving the local
  current user's `reacted_by_me` flags, except when the event was generated by
  the current user; in that case use `reaction_active`.
- The backend reaction allowlist and frontend `AVAILABLE_CHAT_REACTIONS` must be
  kept in sync. Reject arbitrary text or unsupported emoji.

#### 4. Validation & Error Matrix

- Missing/empty emoji -> `ErrInvalidRequest` / HTTP 400-style response.
- Emoji containing whitespace/control characters or exceeding
  `ChatMessageReactionEmojiMaxLength` -> `ErrInvalidRequest`.
- Emoji not in the supported reaction allowlist -> `ErrInvalidRequest`.
- Current user is not a member of the conversation -> `ErrForbidden`.
- Message does not belong to the conversation -> record-not-found style error.
- Message is revoked -> `ErrInvalidRequest`; do not add new reactions to recalled
  messages.

#### 5. Good/Base/Bad Cases

- Good: two members react with `👍`; list messages returns
  `{emoji:"👍", count:2}` and `reacted_by_me` only for the viewing user who
  reacted.
- Good: a current-user WebSocket echo with `reaction_active=false` removes the
  local `reacted_by_me` state or the zero-count reaction pill.
- Base: a message with no reactions serializes `reactions: []`.
- Bad: storing reaction counts in `ChatMessage.Body`; this breaks concurrent
  updates, search/copy semantics, and per-user `reacted_by_me` state.
- Bad: sending a WebSocket `MessageResponse` decorated for the actor to every
  subscriber; this makes other users see `reacted_by_me=true` incorrectly.

#### 6. Tests Required

- `model`: toggling the same `(conversation,message,user,emoji)` adds/removes
  one row and aggregate summaries return counts plus current-user state.
- `service/chat`: toggling requires membership, returns the HTTP actor view, and
  publishes `message.reaction.updated` with aggregate counts plus event metadata.
- `controller`: endpoint smoke test covers HTTP toggle and WebSocket event
  delivery.
- `frontend`: reaction event merge tests preserve local `reacted_by_me` for
  other users and update it for current-user events.
- `frontend`: message content tests ensure sticker/reaction options are pure
  emoji choices, not text/background sticker cards.
...
```go
// Reactions are mutable metadata; do not append them to the message body.
message.Body += "\n<!--reaction:👍:3-->"
```
...
```go
response, active, err := chatService.ToggleMessageReaction(
    ctx,
    currentUserID,
    conversationID,
    messageID,
...
- Waiting text should be refreshed from the original `startedAt` timestamp, not
  ...
  requests.
- Completed task responses should render markdown images using
  `/pg/images/generations/:task_id/image/:index` instead of embedding large
  base64 strings or provider URLs in saved chat messages. This includes task
  items that contain both `url` and `b64_json`: the frontend must still point at
  the task image endpoint so the backend content handler can prefer the stored
  `b64_json` bytes over a stale/source provider URL.
...
- For ChatGPT Web image edits, SSE may first echo a sediment/file reference for
  the uploaded input image. Do not accept a sediment-only SSE reference as the
  final result while active edit references are present. Defer it, mark it as
  excluded for subsequent polling, and poll the conversation/library for the
  generated output instead. This prevents the backend from storing the same
  bytes it uploaded as the edit source.
...
- Completed task item has both `url` and `b64_json` -> markdown uses
  `/pg/images/generations/:task_id/image/:index`; the content endpoint serves
  the base64 bytes instead of redirecting to `url`.
...
- Payload has `image`, `images`, or `reference_images` -> submit to
  ...
  `/pg/images/generations/:task_id`.
- ChatGPT Web image edit SSE returns only a sediment preview while references
  are attached -> do not resolve/download it as the result; continue polling
  while excluding that sediment id.
...
- Good: Playground displays completed task images through the task content
  endpoint even when the raw task item also has a provider `url`, so models such
  as `gpt-image-2` and ChatGPT Web image-capable `gpt-5*` variants do not show
  an input/source URL when `b64_json` holds the generated edit.
- Good: ChatGPT Web image edit receives an immediate SSE `sediment://...`
  preview for the uploaded source, then polls until `file-service://...` or a
  different generated sediment is available; the stored Playground task image
  differs from the source image bytes.
...
- Bad: writing `item.url` directly into completed task markdown when
  `item.b64_json` is also present; the UI can display the original/source image
  even though the backend stored the new generated bytes.
- Bad: treating the first sediment-only SSE image ref in an edit request as the
  final output; ChatGPT Web can echo the uploaded input before the actual
...
- `web/default`: unit tests for completed task `b64_json` -> task image
  markdown URL, including the mixed `url` + `b64_json` case.
- `relay/channel/chatgptimg`: regression test that image edit requests with
  uploaded references poll past sediment-only SSE input previews and return the
  later generated output ref.
...
```go
if len(sseResult.SedimentIDs) > 0 {
    fileRefs = append(fileRefs, "sed:"+sseResult.SedimentIDs[0])
    break
}
```
...
if len(sseResult.FileIDs) == 0 && len(activeRefs) > 0 {
    deferredPreviewSedimentIDs = mergeStringSets(
        deferredPreviewSedimentIDs,
        stringSliceSet(sseResult.SedimentIDs),
    )
} else {
    // accept generated refs normally
}
```
