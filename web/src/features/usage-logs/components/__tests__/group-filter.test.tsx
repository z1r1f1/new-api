...
it('keeps the compact input and masks the dropdown together with other sensitive filters', async () => {
  ...
  const maskedInput = input.closest('.\\[-webkit-text-security\\:disc\\]')
  const maskedDropdown = option.closest('.\\[-webkit-text-security\\:disc\\]')
  expect(maskedInput).not.toBeNull()
  expect(maskedDropdown).not.toBeNull()
