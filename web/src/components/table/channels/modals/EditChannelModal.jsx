...
const normalizeCodexCredential = (value) => {
  ...
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('Codex 密钥必须是 JSON 对象');
  }
  const accessToken = String(parsed.access_token || '').trim();
  const accountId = String(parsed.account_id || '').trim();
  if (!accessToken) {
    throw new Error('Codex 密钥 JSON 必须包含 access_token');
  }
  if (!accountId) {
    throw new Error('Codex 密钥 JSON 必须包含 account_id');
  }
...
const parseCodexBatchCredentialArray = (rawValue) => {
  ...
  if (!trimmedValue.startsWith('[')) {
    throw new Error('Codex 批量密钥必须是标准 JSON 数组');
  }
  const parsedArray = JSON.parse(trimmedValue);
  if (!Array.isArray(parsedArray)) {
    throw new Error('Codex 批量密钥必须是 JSON 数组');
  }
  return parsedArray.map((item) => JSON.parse(normalizeCodexCredential(item)));
};

const parseCodexBatchKeys = (rawValue) => {
  return parseCodexBatchCredentialArray(rawValue).map((item) =>
    JSON.stringify(item),
  );
};
...
const EditChannelModal = (props) => {
  ...
  const originInputs = useMemo(() => {
    if (isEdit) {
      return { ...DEFAULT_NEW_CHANNEL_INPUTS };
    }
    const { id, create_mode, ...presetValues } = props.editingChannel || {};
    return {
      ...DEFAULT_NEW_CHANNEL_INPUTS,
      ...presetValues,
      groups: normalizeStringArray(
        presetValues.groups,
        DEFAULT_NEW_CHANNEL_INPUTS.groups,
      ),
      models: normalizeStringArray(presetValues.models, []),
    };
  }, [isEdit, props.editingChannel]);
  ...
  const [paramOverrideEditorVisible, setParamOverrideEditorVisible] =
    useState(false);
  const formInstanceKey = `${isEdit ? 'edit' : 'new'}-${
    props.editingChannel?.create_mode || 'single'
  }-${inputs.type}-${batch ? 'batch' : 'single'}`;
  ...
  const codexBatchModelsInitializedRef = useRef(false);
  ...
  const resetModalState = () => {
    ...
    codexBatchModelsInitializedRef.current = false;
  ...
  const supportsMultiToSingle = inputs.type !== 57;
                          ...
                          {inputs.type === 57 || inputs.type === 58 ? (
                            ...
                            <>
                              ...
                              {useManualInput && !batch ? (
                                <Form.TextArea
                                  key={`channel-key-${inputs.type}-manual`}
                                  field='key'
                                  label={
                                    isEdit
                                      ? t(
                            ...
                            <Form.Input
                              key={`channel-key-${inputs.type}-single-default`}
                              field='key'
                              label={
                                isEdit
