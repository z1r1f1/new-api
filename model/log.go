...
func applyLogContainsFilter(tx *gorm.DB, column string, value string) *gorm.DB {
	...
	if common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		return tx.Where("positionCaseInsensitive("+column+", ?) > 0", value)
	}
...
func GetAllLogs(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, group string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {
	return GetAllLogsWithFilters(logType, startTimestamp, endTimestamp, modelName, username, tokenName, startIdx, num, channel, "", group, "", requestId, upstreamRequestId)
}

func GetAllLogsWithFilters(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, channelName string, group string, ip string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
...
func GetUserLogs(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, group string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {
	return GetUserLogsWithFilters(userId, logType, startTimestamp, endTimestamp, modelName, tokenName, startIdx, num, 0, "", group, "", requestId, upstreamRequestId)
}

func GetUserLogsWithFilters(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, channel int, channelName string, group string, ip string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
	if logType == LogTypeUnknown {
...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, channelName string, group string, ip string, requestID string) (stat Stat, err error) {
	...
	var totalUseTime, useTimeCount, totalFirstResponseTimeMs, firstResponseTimeCount float64
	...
	if err := timingTx.FindInBatches(&timingRows, 1000, func(tx *gorm.DB, batch int) error {
		for _, row := range timingRows {
			...
			if frt := logOtherNumber(other, "frt"); frt > 0 {
	...
	if logType == LogTypeUnknown || logType == LogTypeConsume {
		...
		cacheTx, err = applyCommonLogStatFilters(cacheTx, LogTypeConsume, startTimestamp, endTimestamp, modelName, username, tokenName, channel, channelName, group, ip, requestID, true)
		if err != nil {
			return stat, err
		}
		var cacheReadTotal, cacheDenominatorTotal float64
