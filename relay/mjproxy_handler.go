...
func serveDataURLImage(c *gin.Context, dataURL string) {
	...
	if comma < 0 || !strings.HasPrefix(dataURL, "data:") {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_data_url"})
		return
	}
	...
	if !strings.Contains(strings.ToLower(metadata), ";base64") {
		c.JSON(http.StatusBadRequest, gin.H{"error": "unsupported_data_url_encoding"})
		return
	}

	imageBytes, err := base64.StdEncoding.DecodeString(payload)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "decode_data_url_failed"})
		return
	}
