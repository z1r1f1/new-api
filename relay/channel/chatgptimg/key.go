...
// OAuthKey 是 ChatGPT 图像渠道使用的凭证载体。
...
func ParseOAuthKey(raw string) (*OAuthKey, error) {
	if strings.TrimSpace(raw) == "" {
		return nil, errors.New("chatgpt image channel: empty oauth key")
	}
	var key OAuthKey
	if err := common.Unmarshal([]byte(raw), &key); err != nil {
		return nil, errors.New("chatgpt image channel: invalid oauth key json")
	}
	return &key, nil
}
...
func ValidateOAuthKeyObject(keyMap map[string]any) error {
	if keyMap == nil {
		return errors.New("chatgpt image key JSON must be an object")
	}
	if !hasNonEmptyValue(keyMap, "access_token") &&
		!hasNonEmptyValue(keyMap, "refresh_token") &&
		!hasNonEmptyValue(keyMap, "session_token") {
		return fmt.Errorf("chatgpt image key JSON must include access_token or refresh/session token")
	}
	return nil
}
...
func NormalizeOAuthKey(raw string) (string, error) {
	...
	if !strings.HasPrefix(trimmed, "{") {
		return "", fmt.Errorf("chatgpt image key must be a valid JSON object")
	}
	...
	if err != nil {
		return "", fmt.Errorf("chatgpt image key JSON 编码失败: %w", err)
	}
...
func GetOAuthBatchKeys(keys string) ([]string, error) {
	...
	if trimmed == "" {
		return nil, fmt.Errorf("batch chatgpt image keys不能为空")
	}
	if !strings.HasPrefix(trimmed, "[") {
		return nil, fmt.Errorf("batch chatgpt image keys must use standard JsonArray format")
	}
	var keyArray []any
	if err := common.Unmarshal([]byte(trimmed), &keyArray); err != nil {
		return nil, fmt.Errorf("batch chatgpt image keys must use standard JsonArray format: %w", err)
	}
	cleanKeys := make([]string, 0, len(keyArray))
	for index, keyValue := range keyArray {
		normalizedKey, err := NormalizeOAuthKeyValue(keyValue)
		if err != nil {
			return nil, fmt.Errorf("第 %d 个 chatgpt image key 无效: %w", index+1, err)
		}
		if normalizedKey != "" {
...
func defaultOAuthChannelName() string {
	return "chatgpt-image-" + strings.ToLower(common.GetRandomString(6))
}
