...
func TestParseChannelCredentialJSONDocumentAcceptsArrayAndStringType(t *testing.T) {
	credentials, err := parseChannelCredentialJSONDocument("many.json", []byte(`[
		...
		{"api_key":"token-b","email":"b@example.com","channel_type":"1"},
		{"access_token":"token-c","account_id":"account-c","email":"c@example.com","type":"chatgptweb"}
	]`))

	require.NoError(t, err)
	require.Len(t, credentials, 3)
	require.Equal(t, constant.ChannelTypeCodex, credentials[0].Type)
	require.Equal(t, constant.ChannelTypeOpenAI, credentials[1].Type)
	require.Equal(t, "b@example.com", credentials[1].Name)
	require.Equal(t, constant.ChannelTypeCodex, credentials[2].Type)
}
...
func TestBuildChannelsFromCredentialImportsAppliesTemplateAndCredentialOverrides(t *testing.T) {
	...
	credentials := []channelCredentialImport{
		...
		{Type: constant.ChannelTypeOpenAI, Name: "b@example.com", Key: `{"api_key":"token-b","email":"b@example.com"}`},
	}
	...
	require.Equal(t, constant.ChannelTypeOpenAI, channels[1].Type)
